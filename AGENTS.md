...
- **There is no test suite.** `gate.sh`'s `ALL` list is the authoritative set of
  harnesses that run and are expected to pass — measured over all 41 entries on
  4.7.1, 39 pass, 1 fail (`boot_state`), 1 flaky (`play_suspends`, which passes
  about half its runs; `bugs.md` 41). This said 23 pass / 1 fail, which does not
  sum to 41, for as long as nobody ran the whole thing and counted.
