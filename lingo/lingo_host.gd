...
func _member_prop_value(which: Variant, cast: String, prop: String) -> Variant:
	...
	if key == "number" or key == "membernum":
		...
		# same question, and only the second had an arm, so the first fell through to
		# 0. The corpus reads it 85 times: every cursor pair in `cursorfunk` resolved
		# to [0, 0], and the failure is silent because 0 is a plausible member number.
