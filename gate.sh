...
echo "corpus: $ROOT"
ALL="game_config root_boot embedded_cast_payload title_mapping title_list export_presets_check preview_surface boot_state:--file@PIP2DATA/EXODUS.DIR frame_events window_preview window_order text_and_shapes text_and_shapes:--root@piposh@--file@PIPDATA/CAPROOM.dir text_and_shapes:--root@piposh@--file@PIPDATA/MAINMENU.dir field_expands:--file@PIP2DATA/SAVELOAD.dir field_expands:--root@piposh@--file@PIPDATA/CAPROOM.dir cursor_preview cursor_hotspot cursor_cross_cast:--root@rating@--boot@mainmenu.dir text_xtra_members text_xtra_members:--root@piposh text_xtra_surface:--root@piposh@--file@PIPDATA/SLOTMACH.dir text_xtra_surface:--file@PIP2DATA/MAP.dir audio_misses container_equality_check lingo_logic_check lingo_designator_check field_designator field_designator:--root@piposh lingo_builtins_check keyboard_check decode_stall hotspots trails sprite_drag debug_bindings snapshot_check container_picker_check go_movie_arg drawn_size_stability sprite_true_colour sprite_true_colour:--root@piposh-dream member_ref_round_trip reg_point movie_churn film_loop_cast film_loop_cast:--root@piposh-dream film_loop_scale film_loop_restart:--root@piposh-dream film_loop_nesting:--root@piposh-dream cast_script_sprite:--root@piposh-dream skip_state wait_frames lingo_fault_sink mouse_events touch_input hilite playhead_escape puppet_persists puzzle_board:--root@piposh-dream hex_board:--root@piposh-dream plane_heading:--root@piposh-dream west_walk:--root@piposh-dream puppet_freeze:--file@PIP2DATA/CHESS.dir@--channel@8@--wheels@138,175@--span@7 editable_text:--file@PIP2DATA/SAVELOAD.dir save_movie:--allow-writes text_codepage save_state sound_wait sound_rate sound_replay_guard sound_linked_member sound_cue_points sound_tempo_wait:--root@rating@--boot@mainmenu.dir key_polling key_overlay:--root@piposh2@--boot@PIP2DATA/ARCADE2.dir@--touch-input key_overlay:--root@rating@--boot@arcade1.dir@--touch-input key_overlay:--root@rating@--boot@NAVIGATE.dir@--touch-input key_overlay:--root@piposh@--boot@PIPDATA/ROULLETE.dir@--touch-input movie_tempo transition_render transition_render:--root@rating@--boot@EGOZROO1.dir film_loop_children:--root@piposh-dream@--boot@plane1.dir script_compile_check script_compile_check:--root@piposh script_compile_check:--root@piposh-en parse_residue lingo_surface_audit lingo_objects lingo_scope_check lingo_local_diagnosis lingo_file_codepage:--allow-writes timeout_and_actors fileio_xtra buddyapi_xtra:--allow-writes media_surface video_fallback avi_decode video_plugin mpeg1_decode lingo_movie_surface property_surface lingo_system_builtins update_stage click_eligibility click_chain primary_scripts sprite_lifetime behaviour_me:--file@PIP2DATA/DAY1.dir behaviour_params frame_reentry:--root@piposh@--file@PIPDATA/CANON.dir@--label@game6 play_suspends play_stack_bound sound_paths sound_folder_scope sound_folder_scope:--root@piposh-dream sound_survey:--root@piposh2@--all fast_forward key_chain mouse_poll:--file@PIP2DATA/CHESS.dir@--label@ches1 sprite_collision collision_arms collision_arms:--root@piposh-dream@--boot@hatul3.dir collision_ink channel_occupancy:--max-frames@120 label_index pause_holds:--file@PIP2DATA/SAVELOAD.dir@--label@savegame2@--hotspot cannon_hit:--root@piposh idle_clock idle_clock:--root@piposh@--boot@PIPDATA/DAY1.dir new_game_reset:--root@rating@--boot@NAVIGATE.dir stage_compare:--root@piposh@--boot@PIPDATA/PIANO.dir@--frame@37@--debug-ui@off@--render@PIPDATA/PIANO.dir@--band@8,466,252,9@--skip-top@60 stage_compare:--root@piposh@--boot@PIPDATA/PIANO.dir@--frame@39@--debug-ui@off@--render@PIPDATA/PIANO.dir@--band@8,466,252,9@--skip-top@60 bitmap_geometry palette_cycle palette_corpus audio_coverage liveness_sweep:--limit@12 launcher_keys launcher_surface"
...
# one of the 82 pairs `collision_ink` measured as changing arm. The first sixteen
...
#
# `collision_ink` is that survey, run bare -- `GATE_ROOT` is the one root of the
# six with **zero** unresolved members, so it is the one where its two guards (the
# ink byte is live, and member resolution is live) mean what they say. 27 s. It is
# in `ALL` rather than left as a one-off because both guards are the difference
# between "no pair changes arm" and "nothing was read", and that is exactly the
# reading `bugs.md` 126 was filed to prevent.
#
