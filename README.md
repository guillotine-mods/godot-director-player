...
godot --headless --script tools/frame_reentry.gd -- --root rating --file NAVIGATE.dir --label thepool --loop-to 542  # where a looping room's playhead goes back to, against what its score says: one destination per loop, and whether the marker's setup frame is inside the loop or entered once per visit. `--loop-to N` pins the frame — without it both arms derive their branch from the measured target and a one-frame move passes on the other arm, which is the whole of what `marker(0)` can get wrong (docs/bugs-closed.md 134). `--score-only --from/--to` prints the score's answer and drives nothing, pass/fail
...
godot --headless --script tools/play_return_frame.gd -- --root rating --boot mainmenu.dir  # an interlude in *another* movie comes back to the frame that called it and enters no other frame of it: the return address is carried into the movie load rather than assigned after it, or the caller's frame 0 runs its own `enterFrame` on the way past (docs/bugs-closed.md 133), pass/fail. Finds its own sites — every frame script under the root holding a `play ... of movie ...` — and prints how many sprite-behaviour sites it left alone (`--only <container>` for one)
...
godot --headless --script tools/exit_leaks.gd       # a run exits with no leaked ObjectDB instances and no resources still in use — boots the real player in a second Godot and reads what the engine says on the way out, beside an in-process check that the Xtra registry does not own its host back. Every run of this project printed `4 resources still in use` until `lingo_xtra.gd` took a WeakRef, pass/fail
godot --headless --script tools/sound_exit_leak.gd # a run that exits **while a sound is still playing** leaks nothing — the case `exit_leaks` cannot reach, because its child never plays anything. A playback whose player leaves the tree is *paused* rather than stopped, so `playing` reads false while the `AudioServer` still holds it, and an `if player.playing` guard skipped the one stop that mattered (`bugs.md` 132), pass/fail
...
godot --headless --script tools/mpeg1_audio.gd  # the MPEG-1 Layer II audio decoder. 23 of its checks need no corpus — ISO Table B.3's window and its peak, the unrolled cosine transform against a direct evaluation of the definition it folds, Table B.4 requantisation for every code of every class, and the allocation table selected **per channel** over all 168 header combinations. Add `--root res://test-games/itamar-magichat` for the only corpus with MPEG audio, which walks 38,060 frames and compares the decode against the Ogg sidecar's Vorbis track
godot --headless --script tools/mask_ink.gd -- --file PIP2DATA/DAY1.dir  # Mask ink (9): the next cast member as a 1-bit stencil. `--scan` finds a usable bitmap/1-bit pair in a root. No sprite record in any of the eight roots carries ink 9 (0 of 8,079,420), so the ink byte is composed on a real member pair — the `sprite_flip.gd` precedent. Its own control asserts the same pixel under Copy ink is untouched, so a decoder merely producing transparent pixels cannot pass
godot --headless --script tools/hit_hole.gd -- --file PIP2DATA/SAVELOAD.dir  # the hit test's third answer: a point in a text member's scrollbar strip aborts the descent rather than continuing it. Asserted where the three readings differ on real sprites — no Hole gives channel 6, Hole-as-Outside gives the sprite behind it, the reference's break gives 0
godot --headless --script tools/hole_survey.gd -- --all  # how many text records have a scrollbar-sized strip, by box type, over every root. 0 `scroll` members exist anywhere, which is why `hit_hole` writes one authoring byte
