...
## 43. `intersects` and `within` measured a hidden sprite as nothing, so nothing in Director's probe idiom ever answered

**Status:** FIXED · **Area:** preview, `lingo_sprite_rect` /
`sprite_state.effective` · reported from play as "in cannon.dir I cannot hit any
of the ships — I can aim and fire, and even when I see the mark on the ships in
the water it doesn't kill them"

**Director asks two rect questions and only one of them consults visibility.**
`channel.cpp:isMouseIn` opens with `if (!_visible) return kCollisionNo`;
`c_within` and `c_intersects` (`lingo/lingo-code.cpp`) go straight to
`getBbox()`. `_visible` is read at **exactly one site** in the whole of
`channel.cpp`, and it is `isMouseIn`. (ScummVM @ `reference/scummvm/REVISION`.)

This port collapsed the two. `lingo_sprite_rect` — which exists only to answer
the two operators — went through `_effective`, and `_effective` returns `{}` for
a sprite a script has hidden. So both operands of every `within` and every
`intersects` measured empty the moment either was invisible, and the host arm's
zero-size guard then answered 0.

**Invisible is not an edge case here, it is how the idiom works.** A script parks
a 1x1 member where it wants to ask a geometric question and asks it. Piposh 1's
cannon round is the corpus's clearest instance — `PIPDATA/CANON.dir` member 496,
`on movecannon`, installed as `the keyDownScript` by the round's own
`enterFrame`:

```lingo
set hitwhere to 316 - (value(the text of field "zavit") * 3)
set the locV of sprite 48 to hitwhere
set the locH of sprite 48 to the locH of sprite 45
updateStage()
repeat with i = 17 to 22
  if sprite 48 within i then ...
```

Channel 48's member is `dot`, 1x1, and the same `enterFrame` does `set the
visible of sprite 48 to 0`. Channels 17-22 are `boatsize`, a shape, placed at
each ship's `locH - 30`. So the shell probe was hidden by design and measured as
nothing, `within` answered 0 six times, and control fell into the `hitwhere <>
"hit"` branch — which paints `minisplash12`/`bigsplash13` at the barrel's `locH`
with the shell's own `locV`. **That is the mark the player saw**: the splash
lands on the ship because the arithmetic is right; only the test said no. Every
shot in the game was a miss, and `krupnikhits` reaching 9 meant the round could
only ever be lost.

**Ruled out along the way**, because each would have produced the same symptom:

- *Stale puppet state.* `sprite_state.effective` discards positional overrides
  when the score moves a channel to another member (the DAY1 channel-6 case), and
  had channel 48 fallen into it the probe would have measured at its score loc,
  down at the cannon. Measured instead: `overrides[48]` reads `{"_member": 463,
  "visible": 0, "locv": 316, "loch": 366}` — the writes survive, `_member`
  agrees, and only the hide is doing damage.
- *Score-rect residue.* Channel 48's score record is 112x118, which could never
  be `within` a 62x28 fence. It never applies: the member is 1x1 and the stretch
  flag is clear, so `sprite_geometry.drawn_size` already returns the natural size
  (entry 31's rule). The rect comes out 1x1.
- *The matte branch.* `c_within` only compares mattes when **both** sprites are
  non-QD-shape with `kInkTypeMatte`. The fences are shapes, so the reference
  takes `getBbox().contains(getBbox())` — plain rects, which is what this port
  does.
- *The keyboard.* The angle and the barrel both moved, so `the keyDownScript`,
  `the keyCode` and the field write were all already working.

**The fix is one flag, not a second merge path.** `effective` gains
`ignore_visible`, honoured by `lingo_sprite_rect` alone. A duplicated rect
function would have drifted, because `effective` *mutates* `overrides` in the
stale-puppet discard.

**One deliberate behaviour change beyond the operators.**
`interaction.constraint_box` reads `lingo_sprite_rect`, so `the constraint of
sprite` now clamps against a hidden constraint channel where before it read as
unconstrained. That matches the reference — `channel.cpp:698` reaches the
constraint through `getRollOverBbox()`, which is `getBbox()`. An *empty* channel
...
`sprite_drag` — the eleven near-copies of `sprite the clickOn intersects 100`
that the old comment was written for — passes unchanged on piposh2.
