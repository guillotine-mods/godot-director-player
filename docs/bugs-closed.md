...
| 110 | **The report was the bug, and then the sorting falsified itself** | All eight are `vectorShape` Xtras. `"child has no art"` was one bucket holding an undrawable member type and a decode failure; `sprite_art.gd:decline_reason` splits them and `tools/film_loop_children.gd` holds the painter to a census derived off the cast. That much stands. What did not: with the symbol finally named in the report it was plain that `vectorShape` is **Director's own vector art**, not an unregistered third-party Xtra, so "neither this port nor the reference draws it, correctly" was right about the reference and wrong about what this port should do. All eight are drawn as of 2026-08-29 — see `director/director_vector_shape.gd` and `tools/vector_shape.gd`. |
...
# 83, 45 and 118, closed 2026-08-15

**45 is the one to read.** Its premise was false, and correcting it turned the
verdict from "unreachable" to "reachable, reproduces, and is the original's own
defect".

| # | Verdict | What settled it |
|---|---|---|
| 83 | **Fixed** | `initializerIndex` is read, the entry opened, the string cut at the NUL exactly as `Common::ReadStream::readString()` does, and the pairs seeded onto the instance at `beginSprite` — `Score::createScriptInstance`'s position. A pair the script never declared is **dropped, not invented**, matching `ScriptContext::setProp`'s `force = false`. |
| 45 | **Not ours. Authentic 1997 defect.** | There is no re-entry: the marker `game6` **is** frame 349 and it is the setup frame; the loop is 350..385 and never touches it. `bugs.md`'s narrowing read the marker as 348 — the 0-based index `Labels` stores — which is where "the entry frame re-runs" came from. |
| 118 | **Decided: `window_size()`, structurally** | `Window::playTransition` is a method on the window the frame change happened in; both pictures are allocated at `composeSurface->w/h` from `_window->getSurface()` and a changed-area clip is clipped to `_window->getInnerDimensions()`. No path in the reference sizes a window's transition by the stage. |

## 83: how many spans actually carry one, and the second defect that found

`tools/behaviour_params.gd --survey` over all eight roots: 677 containers, 491
scores, **127,559 behaviour elements — 82 spans carry an authored initialiser, and
every one is `test-games/itamar-magichat`'s.** Zero in all six shipped titles and
in `itamar-park`. Built anyway, per "Build Director, not this game".

**The survey turned up a decode defect nobody was looking for.** Initialiser
entries are Lingo *source*, and a long one clears the 44-byte floor `parse()` uses
to decide an entry is a span record — `[#prSprite: 7, …]` is 100 bytes in
`trivia.dir`. So the scan was reading **text as sprite spans**. `parse()` now skips
an entry that some element named as an initialiser: **122 non-span intervals
removed, all `itamar-magichat`'s, 0 in the six shipped titles**, so no shipped
decode can change and the survey asserts the difference is never negative.

The reverted control is the vivid one — with the decode disabled, the harness
reports five intervals where there are two, one of them
`sprite ch 1886540650 [1529049201..1399878248] member 28786`. That is what reading
a Lingo string as a span record looks like.

## 45: a one-frame off-by-one in the entry, and what it was hiding

`tools/frame_reentry.gd --score-only` on `CANON.dir`, in 1-based frames: `game6` is
**349**, the setup frame carrying member 640's `on enterFrame`; 350..384 is member
641's `on exitFrame`; 385 is member 27, whose entire body is `go(marker(0) + 1)` →
`go(350)`. Driven for 1400 awaited frames: `frame 349 x 1` against eight or nine
passes of 350..385, and `lingo ran : {"enterFrame":1,"exitFrame":114}`. **Nothing
resets `allshipscounter`.**

So the drift is reachable, and it reproduces:

```
tick 62  ch10 locV 90 -> -74 (-164), top -102, counter 0,10,0
FAIL  every submarine is still inside the stage after the countdown
      (lowest top -102 on channel 10)
```

An unpaired surface from a counter armed by a hit. Member 641 subtracts whenever
the counter reaches 1 and **never asks which of its two writers set it** — and
member 642 writes `put 20 into item i - 16 of allshipscounter` in all four hit
stages. One global, two purposes, in the movie's own script. A faithful Director
lifts the sub too, so by this entry's own criterion it is the original's defect and
not the engine's.

`tools/frame_reentry.gd` has **no reverted control and says so**: 45 required no
code change, so it guards a verdict rather than a fix.

## 118: equal in all eight corpora, which is why nothing could fail

Three call sites asked three different questions — `paint_capture` by
`window_size()`, `_grab_stage` and `begin_transition` by `stage_size()`. The two
that were wrong are now `window_size()`, and it is a no-op on the stage **by
construction rather than by corpus**: `windows.gd:size_of` falls back to
`stage_size()` for a node with no `_window_rect`, and the stage node never has one.
...
MPEG decoder.
**Closed 2026-08-15. The headline was retracted, the rect was the wrong field, and
what was actually missing was `decodeXMED`.**

The six members the entry named are `flash` and `VisibleLightOnStageMedia`, and
`castmember/xtra.cpp:41-47` registers exactly four symbols — `cursor`,
`quickTimeMedia`, `text`, `font` — everything else falling through
`XtraCastMember::promote` to `CastMember::createWidget` returning `nullptr`. **The
reference draws none of them either.** Corpus-wide, 555 of 566 Xtra members are
unregistered there.

**The 0x0 `xtraRect` was the wrong field, not a decode gap.**
`_apply_xtra_rect` reads item 12 of the *info* block, and the `text` Xtra does not
put geometry there: `TextXtra::parseXtraData` reads a big-endian height at offset
**36** of the Xtra's own payload and a width at **40**, refusing below 44 bytes or
outside `1..0x4000`. All 11 pass those bounds and now report their real size.

## `XMED` is not a Director format

It is a serialised **Hermes Paige** document; Director stores the chunk and never
opens it. What the reference knows: four ASCII bytes `FFFF` at the head, a body of
mostly ASCII hex, and a **literal run** written as `0x00`, up to six hex digits of
length, `,`, then that many raw bytes. **The first literal run that is ≥ 4/5
printable is the text** — printable being ASCII `0x20..0x7e`, *anything ≥ 0x80*,
and CR/LF/TAB.

**Both halves of that ratio earn their keep and neither is a guard.**
`SLOTMACH.dir` #83's chunk carries a 64-byte run at offset 487 reading
`05 e7 f8 ee e5 ef 00 00 …` — a Pascal string in a fixed buffer holding the Hebrew
font name `קרמון`. 5 printable in 64 fails the ratio, and that failure is the only
thing stopping the member's *font* being returned as its *text*. The `≥ 0x80` arm
is equally load-bearing: 5 of the 11 are Hebrew and every byte of their text is
above 0x7f.

## What the eleven turned out to be

11 of 11 decode; none refused. `piposh`/`piposh-en`/`piposh-ru SLOTMACH.dir` #83
`credit` is the single character `0`. `piposh-dream`'s three maze members hold the
same 437-character Lingo **scratchpad** (`pathx=[] … setat(pathx,count(pathx)+1,23)`).
`piposh2 HEZSAVE.DIR`/`AIR1.dir` #118 `temporary` holds two `put the moviepath &`
lines. `MAP.dir` #12 is a room-name-to-coordinate table and #17 is the same table
commented out. `rating ARCADE1.dir` #136 is `objectsfound contains : 1 joke from
recep …`.

That they are **recognisable things** — authoring notes, a coordinate table, a
credit readout — is what makes the first-passing-run heuristic checkable rather
than merely plausible.

## The collision, and why the assertion could not pass by accident

`TextXtraCastMember` sets `_type = kCastXtra`, so a promoted `credit` keys as
`credit:15` while `field "credit"` asks for `credit:3`. Measured in all three
Piposh 1 builds: `number_of('credit') = 83` and
`number_of_type('credit', field) = 97`, unchanged by promotion. **The `02844f93`
slot-machine fix holds for the same structural reason the reference holds it.**

The field's authored text is *also* `0`, so comparing values proves nothing.
Writing `xtra-probe-8547` into the Xtra and finding the field still `0` — and the
reverse — is what proves there are two stores. That is asserted inside a running
`SLOTMACH.dir`.

**One piece stated rather than hidden**: `TextXtraCastMember::createWidget` builds
a `MacText`, so the reference *draws* a promoted member, and `sprite_art.gd`
...
**Resolved 2026-08-22, and by the route this entry itself named.** All three
pieces landed: the vocabulary, the boundary, and then the abort.

## 1. The discriminator is now a statement about Director

`lingo/lingo_director_names.gd` is a new table of names **Director documents**,
family by family and sourced in the file, held deliberately apart from
`lingo_reference_names.gd` — which answers the *other* question, being the union
of the four tables `LC::call` consults. The entry's whole bind was that only the
second existed, so "the reference has no entry" meant either "Director has no such
word" or "ScummVM has not implemented it", and nothing at the call site told them
apart.

Re-measured, `tools/undefined_calls.gd --all` over the six shipped roots, 91,737
call sites:

```
resolved 14162, reference-known 77556, Director-known 2, UNDEFINED 17
6 distinct undefined name(s): displayobject, dont, gamad, mraker, setmoviepath, www
```

**`gotoNetPage` moved to Director-known and `mraker` stayed undefined** — the two
cases the entry said nothing could separate, separated. 19 sites over 7 names
became 17 over 6.

## 2. The boundary, which the entry called its own piece of work

`_abort` is scoped to **one `execute()` call and not one event**: `b_call`,
`callBehaviorHandler` and `sendAllSprites` each re-enter `execute(frame)`, and
this port had no such boundary at all because `_broadcast` is an ordinary GDScript
call. Implementing the abort without it **would have unwound further than Director
does** — a worse bug than the divergence being fixed.

`begin_execute`/`end_execute` are that boundary, and
`tools/lingo_execute_boundary.gd` asserts it in 20 checks. The two that carry the
meaning:

```
ok  the message ran and aborted where it said it would
    (count 1; 101 would mean the abort did not stop the recipient at all)
ok  and the caller ran its next statement  (gBoundaryAfter 1)
```

Stopping the recipient *and* resuming the caller is the whole of
`Lingo::execute`'s epilogue — it pops every remaining `CFrame` and then clears
`_abort`, so the scope's caller carries on. Note also that `LC::procret` sets the
same flag on an ordinary return, so `_abort` means **"stop the loop"** and not "an
error happened"; a port that read the flag's name literally would produce the
wrong control flow.

## 3. Only then, the abort

`_aborting = true` on the undefined fall-through, reached through `_fail` so it
still prints. Reference-known names answer exactly as before, so `getPref`,
`externalParamName`, `externalParamValue` and the rest of that class are
untouched.

## What the entry's own bare-word half turned up

218 statements went through a `_read_var` path with no abort in it and nobody had
looked. Measured now: **one distinct undefined bare name across all six roots,
`end`.**

## Provenance of this closure, stated because it matters

The agent that did the work **stalled and was never able to report**, so there is
no narrative account of its reasoning beyond what it wrote into the files, and
**no reverted-control output survives**. What is verified here was verified
directly: both harnesses run green on demand (`undefined_handler` 20 checks,
`lingo_execute_boundary` 20), the re-measurement above was re-run rather than
quoted, and the interpreter change was gated across 23 entries covering the whole
Lingo surface, the event chain, the play stack and the frame clock.

**Not verified, and the entry's own standing coverage gap is therefore still
open**: nobody has seen `UNDEFINED_HANDLER` fire on real corpus data. The
`mraker` arm sits behind `if not soundBusy(1)`, and `liveness_sweep --speech` is
the flag that would get past it.
