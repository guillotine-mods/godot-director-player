...
Expected: a named `FAIL  install_godot.sh parses` line, then the three asset
checks, then `FAIL  install_godot`. The named FAIL is the point: `bash -n` on a
missing file exits non-zero, and with the probe guarded that becomes a reported
failure rather than an abort. If instead you see only the "suite aborted" line,
a probe is unguarded.
