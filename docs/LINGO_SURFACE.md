...
| 12 | `the currentSpriteNum` | **live** | Piposh Dream's hex board reads it to know which channel was clicked, and **every one of the 12 sites in the six titles is a cast script on a bitmap member — 0 are behaviours.** This row said the opposite ("which channel is running the behaviour, once per tile") and the implementation was built on that reading, so the property answered 0 at every site that exists and the Hexxagon board could not be played: members 3, 103 and 105 of `hex1`/`hex2`/`hex3` do `jumpFrom = the currentSpriteNum` and `lightUp1Hex(0)` matches no `case` arm. §7.1 says it is synthesised rather than stored, and it is: the host carries `current_sprite_num`, `preview/event_chain.gd` sets it per queued element and puts back what it found, and `sendSprite`/`sendAllSprites` bracket their sends the same way so a broadcasting behaviour reads its own channel again on the way back. The two tiers that belong to a sprite — the behaviour and its member's cast script — answer the channel; a frame script and a movie script read 0. That **departs from the reference** at the cast tier, deliberately: ScummVM fills `_currentSpriteNum` only in `processEvent`'s `kScoreScript` arm and a mouse chain queues every other tier with `channelId = 0`. `preview/event_chain.gd:element` carries the argument, the citation and how to reverse it. Asserted by `tools/cast_script_sprite.gd`. |
