on whatodoeveryframe3
  global whatodo, egozh, egozv, upordown, stopornot, nextroomdata, syz, resizepip, hope, ifmovie
  if whatodo = "walktime" then
    if member(the memberNum of sprite 30).name contains "walkleft" then
      ...
      if (the locV of sprite 30 - 10) < egozv then
        set the memberNum of sprite 30 to the number of member ("walkleft" & x) of castLib 1
        set the locV of sprite 30 to the locV of sprite 30 - 6
      else
        set the memberNum of sprite 30 to the number of member ("walkleftup" & x) of castLib 1
        set the locV of sprite 30 to the locV of sprite 30 - 6
      end if
    else
      if member(the memberNum of sprite 30).name contains "walkright" then
        ...
        x = value(item 3 of stopornot)
        if (the locV of sprite 30 - 10) < egozv then
          set the memberNum of sprite 30 to the number of member ("walkright" & x) of castLib 1
          set the locV of sprite 30 to the locV of sprite 30 + 6
        else
          set the memberNum of sprite 30 to the number of member ("walkrightup" & x) of castLib 1
          set the locV of sprite 30 to the locV of sprite 30 + 6
        end if
      end if
    end if
    ...
    if whatodo = "stand" then
      if member(the memberNum of sprite 30).name contains "walkright" then
        set the memberNum of sprite 30 to the number of member "standright" of castLib 1
      else
        if member(the memberNum of sprite 30).name contains "walkleft" then
          set the memberNum of sprite 30 to the number of member "standleft" of castLib 1
        end if
      end if
      if nextroomdata <> "000" then
        if value(item 2 of nextroomdata) > 300 then
          ...
          set the memberNum of sprite 30 to the number of member "standright"
        end if
