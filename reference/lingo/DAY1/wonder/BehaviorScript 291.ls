on exitFrame
  ...
  if usfultalking = 1 then
    ...
    if r <> "not" then
      ...
      if item 2 of line usfulline of field fieldname of castLib "master" = "0" then
        sound playFile 1, soundspath & "d" & globalday & "prom" & usfultalking & ".aif"
        ...
        sound playFile 1, soundspath & "tofsay" & random(3) + vcv & ".aif"
        usfultalking = 100
      end if
    end if
  else
  ...
  if soundBusy(1) then
    ...
    if the locH of sprite 30 > 534 then
      hezzz = "left"
    else
      hezzz = "right"
    end if
    ...
    if who contains "tof" then
      ...
      if who contains "mov" then
        ...
        set the memberNum of sprite rin to the number of member (xxx & "toflop1") of castLib "wonder"
      end if
    end if
  end if
end
