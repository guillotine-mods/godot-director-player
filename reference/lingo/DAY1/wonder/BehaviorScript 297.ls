on exitFrame
  ...
  if item 4 of line 6 of field "Dprocess" of castLib "master" <> "done" then
    x = "continue"
    repeat with i = 1 to the number of lines in field "objectsfield" of castLib "master"
      if (line i of field "objectsfield" of castLib "master" = "empty") and (x = "continue") then
        put "photo" into line i of field "objectsfield" of castLib "master"
        x = "stop"
      end if
    end repeat
    put "done" into item 4 of line 6 of field "Dprocess" of castLib "master"
    ...
    displayobject()
    sound playFile 1, effectspath & "camera.aif"
  end if
  play done
end
