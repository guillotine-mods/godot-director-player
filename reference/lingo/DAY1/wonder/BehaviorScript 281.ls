on exitFrame
  ...
  lastmark = "exitforest3"
  usfulobject = "dnzday" & globalday
  if usfultalking = 1 then
    ...
    if r <> "not" then
      ...
      if item 2 of line usfulline of field fieldname of castLib "master" = "0" then
        sound playFile 1, soundspath & "j" & globalday & "prom" & usfultalking & ".aif"
        put 1 into item 2 of line usfulline of field fieldname of castLib "master"
        usfultalking = usfultalking + 1
      else
        sound playFile 1, soundspath & "jsay" & random(6) & ".aif"
        usfultalking = 100
      end if
    end if
  else
  ...
  if soundBusy(1) then
    ...
    if who contains "dnz" then
      ...
      if who contains "mov" then
        sprite(rin).visible = 0
        play frame who
        sprite(rin).visible = 1
      else
        if who contains "dididi" then
          set the memberNum of sprite 30 to the number of member ("pele" & syz) of castLib 1
          set the locV of sprite 30 to egozv
          set the locH of sprite 30 to egozh
          set the memberNum of sprite rin to the number of member (xxx & "dnzspk1") of castLib "wonder"
        else
          if who contains "call" then
            case syz of
              9:
                hezspeaks = "13,-123"
              8:
            ...
            set the memberNum of sprite 30 to the number of member ("call" & syz) of castLib 1
            set the memberNum of sprite rin to the number of member (xxx & "dnzlop1") of castLib "wonder"
          else
            if hezzz = "left" then
              case syz of
                9:
