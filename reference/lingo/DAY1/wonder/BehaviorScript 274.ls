  ...
  if usfultalking = 1 then
    ...
    if r <> "not" then
      ...
      if item 2 of line usfulline of field fieldname of castLib "master" = "0" then
        sound playFile 1, soundspath & "i" & globalday & "prom" & usfultalking & ".aif"
        put 1 into item 2 of line usfulline of field fieldname of castLib "master"
        usfultalking = usfultalking + 1
      else
        sound playFile 1, soundspath & "isay" & random(6) & ".aif"
        usfultalking = 100
      end if
    end if
  else
  ...
  if soundBusy(1) then
    ...
    if who contains "lil" then
      ...
      if who contains "mov" then
        ...
        set the memberNum of sprite rin to the number of member (xxx & "lillop1") of castLib "wonder"
      end if
    end if
  end if
end
