on exitFrame
  ...
  if usfultalking = 1 then
    ...
    if r <> "not" then
      ...
      if item 2 of line usfulline of field fieldname of castLib "master" = "0" then
        sound playFile 1, soundspath & "a" & globalday & "prom" & usfultalking & ".aif"
        put 1 into item 2 of line usfulline of field fieldname of castLib "master"
        usfultalking = usfultalking + 1
      else
        d = random(2)
        if d = 1 then
          sound playFile 1, soundspath & "rinsay" & random(3) & ".aif"
          usfultalking = 100
        else
          sound playFile 1, soundspath & "patsay" & random(5) & ".aif"
          usfultalking = 101
        end if
      end if
    end if
  ...
  if soundBusy(1) then
    ...
    if usfultalking = 100 then
      ...
      if usfultalking = 101 then
        who = "patspk2"
      end if
    end if
    if (who contains "rin") or (who contains "pat") then
      set the memberNum of sprite 30 to the number of member ("stand" & hezzz & syz) of castLib 1
      set the locV of sprite 30 to egozv
      set the locH of sprite 30 to egozh
      if who contains "rin" then
        go(marker("rinclicktalk") + 1)
      else
        go(marker("patclicktalk") + 1)
      end if
    else
