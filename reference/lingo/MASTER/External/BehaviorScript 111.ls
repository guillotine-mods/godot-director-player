on mouseUp
  ...
  if sprite the clickOn intersects 100 then
    ...
    r = "not"
    if sprite the clickOn intersects 15 and (sprite(15).visible = 1) then
      r = "yes"
    end if
    if r = "yes" then
      ...
      if x = "sciser" then
        ...
        play frame "cutmirolohair"
      else
        sound stop 2
        sound playFile 1, soundspath & "mirolo" & random(4) + 1 & ".aif"
        set the locH of sprite the clickOn to objectxx
        set the locV of sprite the clickOn to objectyy
        updateStage()
        set the memberNum of sprite 100 to the number of member "piphead1" of castLib "master"
        play frame "mirolospk"
      end if
    end if
  end if
  set the locH of sprite the clickOn to objectxx
