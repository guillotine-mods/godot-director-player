on exitFrame
  global soundspath
  if item 4 of line 3 of field "Dprocess" <> "done" then
    x = "continue"
    repeat with i = 1 to the number of lines in field "objectsfield" of castLib "master"
      if (line i of field "objectsfield" of castLib "master" = "empty") and (x = "continue") then
        put "spring" into line i of field "objectsfield" of castLib "master"
        x = "stop"
      end if
    end repeat
    put "done" into item 4 of line 3 of field "Dprocess" of castLib "master"
  ...
  sound playFile 1, soundspath & "pfspring.aif"
end
