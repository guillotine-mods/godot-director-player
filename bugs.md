...
**Cross-reference for whoever picks up entry 93.** `scenes/preview/event_chain.gd`
now separates the two channel notions — the channel a behaviour runs *as an
instance* (`channel`) and the channel an element runs *for* (`sprite`), see
`docs/bugs-closed.md` 104's neighbour, the `currentSpriteNum` fix. Fixing 93 by
widening the instance `channel` would silently turn cast scripts into behaviour
instances, which is precisely what that split exists to prevent.
